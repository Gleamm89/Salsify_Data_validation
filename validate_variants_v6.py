...
#
# Main HTML report:
#   validation_group_details.html shows:
#   - all SKUs inside every Group_ID that has an issue
#   - original import columns: Variation_Value_1/2/3 - en
#   - interpreted columns using Variation_Name_1/2/3 - en where available
...
def build_group_details_html(html_file):
    ...
    for group_id in sorted(issue_group_ids):
        ...
        for row in child_rows:
            ...
            raw_value_cells = "".join(
                f"<td>{html.escape(value)}</td>"
                for value in raw_values
            )

            # Same values again, but displayed under interpreted variation names.
            interpreted_value_cells = "".join(
                f"<td>{html.escape(value)}</td>"
                for value in raw_values
            )
...
<body>
    <div class="summary">
        ...
        <p>This page includes all SKUs in every Group_ID that has at least one issue.</p>
    </div>
    {''.join(group_sections)}
</body>
</html>"""
